...
- **Self-service format & scoring (active season only)**: A matchup's two participants (or an admin) can, from either the Playoffs page or the Leaderboard's Playoffs tab:
  ...
  - **For stroke play**: no manual entry is needed — each participant's live best-net score (lowest net for 18-hole matchups; sum of the two best nets for the 36-hole championship) is computed automatically from posted scores in the matchup's playoff event (`resolveBestNet` / `resolveEventIdForMatchup`) and displayed to everyone viewing the active season's bracket, not just participants.
  - Self-service writes go through `SECURITY DEFINER` RPCs (`set_playoff_matchup_format`, `upsert_playoff_match_hole`, `set_playoff_match_status`) so participants can update their own matchup without needing admin/service-role access, while historical (non-active) seasons remain fully read-only.
- The admin's Player 1 / Player 2 / Winner controls remain the single official source of truth for advancement — assigning a winner is always admin-only, independent of whether participants used self-service to play out the match. A DB trigger + app-side guard prevent an orphaned `winner_id` (one that doesn't match either participant) from ever being persisted.
...
- **Event types**:
  ...
  - `playoff_match_final` — A matchup was decided: fires from the self-service "Mark Match Final" action for match play (winner + closeout inferred from the hole log), or from the admin assigning a winner for the first time (the only trigger for stroke play, and a backstop for match play) — on by default, guarded against re-firing on later corrections
