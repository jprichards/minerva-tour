...
- **Self-service format & scoring (active season only)**: A matchup's two participants (or an admin) can, from either the Playoffs page or the Leaderboard's Playoffs tab:
  - **Choose the format** (stroke play or handicapped match play) and hole count (18 or 36) for their matchup, once, via a format/holes picker shown until a format is set. Both formats default to the rulebook's "best net" stroke play when undecided.
  - **For match play**: log each hole's result (Player 1 / halve / Player 2) via a hole-by-hole grid sized to the matchup's hole count, labeled with each player's first name (e.g. "David" / "Halved" / "Grady") rather than generic P1/P2/AS. Running status ("4 UP thru 7", "3 & 2", "All Square (halved)") is computed live from the hole-by-hole log (`src/lib/playoffs.ts` — `computeMatchStatus`) and mirrored into the same `player1_result` / `player2_result` text fields the admin bracket view already displays, so both self-service and admin views always agree. Once holes have been logged, participants can mark the matchup **Final** (does not set the official `winner_id` — that stays admin-only, see below). The hole-by-hole grid itself is visible to **any** viewer (spectators included, labeled "View holes") once a format is match play, but only the matchup's two participants and admins see it as editable ("Log holes") — spectators get the same grid read-only, with no tap targets and no Mark Match Final button.
  - **For stroke play**: no manual entry is needed — each participant's live best-net score (lowest net for 18-hole matchups; sum of the two best nets for the 36-hole championship) is computed automatically from posted scores in the matchup's playoff event (`resolveBestNet` / `resolveEventIdForMatchup`) and displayed to everyone viewing the active season's bracket, not just participants. Once both players' best-net scores are posted and there's a clear leader (no tie), either participant or an admin can tap **Confirm Winner** to set the official `winner_id` directly and mark the matchup final — a self-service exception to the admin-only winner rule below, since stroke play has no hole-by-hole log to fall back on the way match play's self-service Mark Final does. The button disappears immediately once used (on either device) or if the nets are tied (the note instead says an admin will need to resolve it).
  - Self-service writes go through `SECURITY DEFINER` RPCs (`set_playoff_matchup_format`, `upsert_playoff_match_hole`, `set_playoff_match_status`, `confirm_stroke_play_winner`) so participants can update their own matchup without needing admin/service-role access, while historical (non-active) seasons remain fully read-only.
- The admin's Player 1 / Player 2 / Winner controls remain the single official source of truth for advancement — assigning a winner is admin-only for match play, with the one exception above for stroke play's self-service Confirm Winner (which is itself restricted server-side to `stroke_play` matchups and to one of the two actual participants). A DB trigger + app-side guard prevent an orphaned `winner_id` (one that doesn't match either participant) from ever being persisted, regardless of which code path sets it.
- **Admin format override** (`/admin/playoffs` edit form): once a format is self-service-selected, the format picker never reappears for participants — the format is sticky by design. The admin edit form is the only way to correct or reset it, via a Format select ("Not set (auto best-net)" / "Stroke Play" / "Match Play") and a conditional Holes select (18/36). Changing the format resets `status` back to `scheduled`; resetting to "Not set" or switching away from Match Play also deletes any logged `playoff_match_holes` rows for that matchup, since hole-by-hole data is meaningless without a match-play format. A matchup's current format/holes, when set, is also shown as a small badge on the admin bracket display so this state is visible without opening edit mode.
...
- The bracket is a shared component (`PlayoffBracket` + `MatchupCard`) rendered both on the standalone Playoffs page and as a "Playoffs" tab alongside "Net" / "Scratch" on the Leaderboard page, so the two stay visually and behaviorally in sync. The Leaderboard's Playoffs tab only appears for seasons that have at least one playoff bracket.
- Flight tabs (Championship / Consolation / Unicorn) are only shown when a season has brackets in more than one flight. Round labels (Quarterfinal / Semifinal / Final / Round N) are derived from each flight's seed count, not just from whichever rounds happen to exist yet, so a lone first round never misreads as "Final".
- Winner/loser styling only applies when `winner_id` matches one of the matchup's two participants; an orphaned winner (e.g. after a player re-assignment) is treated as undecided rather than mis-highlighting a slot.
...
- **Event types**:
  ...
  - `playoff_format_set` — A matchup's participants (or an admin) chose a format/hole count via self-service (off by default — low-key/informational)
  - `playoff_match_start` — The first hole was logged for a match-play matchup (announces the matchup + format; on by default)
  - `playoff_status_update` — A subsequent match-play hole was logged, with the running status text and hole number (on by default)
  - `playoff_stroke_score` — A playoff participant posted a stroke-play score; additive to (never a bypass of) the regular score notifications and gated by the same `score_post_trigger` cadence (off by default to avoid doubling up with regular score posts)
  - `playoff_match_final` — A matchup was decided: fires from the self-service "Mark Match Final" action for match play (winner + closeout inferred from the hole log), from the self-service "Confirm Winner" action for stroke play (winner + net score margin), or from the admin assigning a winner for the first time (a backstop for either format) — on by default, guarded against re-firing on later corrections. **Seed-selection exception**: the round-1 matchup between a flight's top 2 seeds (championship seeds #1/#2, consolation seeds #7/#8 — both get a bye but play each other only to decide who picks their round 2 opponent, per the Playoffs section above) is auto-detected purely from `round` + seed number (`isSeedSelectionMatch` in `src/lib/playoffs.ts` — no separate matchup-type flag) and gets different copy that doesn't say "defeats"/"advances", since both players advance regardless of this result
  - `playoff_round_complete` — Every matchup in a round (flight-scoped) is decided — bye, self-service `status: 'final'`, or admin-assigned `winner_id` all count — fires once from whichever action completed the round; on by default
  - **Round label consistency**: both `playoff_match_final` and `playoff_round_complete` include the same Quarterfinal/Semifinal/Final/Round N label a player would see in the bracket UI, regardless of which flow decided the matchup. Self-service flows (`MatchupCard.tsx`) already have this label rendered on-screen and pass it straight through; the admin page has no rendered bracket to read a label off of, so it derives the same label from the flight's seed count via `computeRoundLabel` in `src/lib/playoffs.ts` (the same round-count math as the bracket UI) rather than falling back to a bare "Round N"
- **Score post trigger** (`slack_config.score_post_trigger`): Controls when score-related Slack messages are sent at all — **every score update** (default, preserving existing behavior), **9 holes complete** (at the turn for 18-hole rounds, at completion for 9-hole rounds), or **round complete only**. This is independent of the chirp trigger — the score post trigger gates the entire Slack message, while the chirp trigger only gates the chirp line within it. `playoff_stroke_score` respects this same trigger (via `is_complete`/`holes_played` on the playoff payload) so it never fires more often than the regular-season score settings allow.
- **Playoff notification defaults & toggles**: All 6 playoff event types default per the list above and are independently toggleable in `/admin/settings` under a "Playoff Notifications" section, alongside the existing score/feedback toggles. A stored config saved before a given playoff key existed still gets that key's intended default (via `DEFAULT_SLACK_EVENTS` merge in `src/lib/slack.ts`) rather than silently defaulting to enabled.
- **Playoff message routing**: Playoff events skip all score-specific enrichment (no projected points, no chirps, no `score_post_trigger` gating for the 5 non-stroke-score playoff types) in the `/api/slack/notify` route — they're a separate branch from regular score posts.
- **Architecture**: Client fires a fire-and-forget POST to `/api/slack/notify` after score operations, feedback submissions, and playoff self-service/admin actions. The API route reads the Slack config from `app_settings` server-side (bot token never exposed to browser), checks if the event type is enabled, applies the **score post trigger** to gate score-related messages, reads **`chirp_config`** to decide whether to attach a chirp for this notification (round-complete-only, 9-holes-complete, or all score updates), **consumes** the next chirp from the matching bucket when the **`chirps-queue`** flag is on (ordered queue + archive + replenishment), otherwise picks a **random** pool template, formats a Slack Block Kit message, and posts via `chat.postMessage`.
...
- **Configuration stored in** `app_settings`: `slack_config` (JSONB) holds bot_token, channel_id, channel_name, per-event-type enabled flags, optional feedback_channel_id/feedback_channel_name, and optional `score_post_trigger`. **`chirp_config`** and **`chirp_ai_config`** (separate keys) hold chirp trigger mode and AI generation settings for queue replenishment.
