...
- **Score post trigger** (`slack_config.score_post_trigger`): Controls when score-related Slack messages are sent at all — **every score update** (default, preserving existing behavior), **9 holes complete** (at the turn for 18-hole rounds, at completion for 9-hole rounds), or **round complete only**. This is independent of the chirp trigger — the score post trigger gates the entire Slack message, while the chirp trigger only gates the chirp line within it.
