...
- **Self-service format & scoring (active season only)**: A matchup's two participants (or an admin) can, from either the Playoffs page or the Leaderboard's Playoffs tab:
  ...
  - **For match play**: log each hole's result (Player 1 / halve / Player 2) via a hole-by-hole grid sized to the matchup's hole count. Running status ("4 UP thru 7", "3 & 2", "All Square (halved)") is computed live from the hole-by-hole log (`src/lib/playoffs.ts` — `computeMatchStatus`) and mirrored into the same `player1_result` / `player2_result` text fields the admin bracket view already displays, so both self-service and admin views always agree. Once holes have been logged, participants can mark the matchup **Final** (does not set the official `winner_id` — that stays admin-only, see below).
