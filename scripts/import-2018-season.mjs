/**
 * Import 2018 season data from "(2018) Minerva Tour Stats and Scores.xlsx".
 *
 * Similar to 2019 format but:
 *   - No 2018 season in DB yet -- creates it automatically
 *   - Event names: "Event 3 (Major - 18 Holes)", "fEvent 10 (Playoffs - 9 Holes)",
 *     "hEvent 12 (Tour Championship - 18 Holes)"
 *   - Score Type includes "offseason" rows (no event) -- skipped
 *   - Some rows have empty event column -- skipped
 *   - Has Par column at [16], Over Par at [14], Thru at [15]
 *
 * Usage:
 *   node scripts/import-2018-season.mjs [--dry-run]
 */
...
const XLSX_PATH = resolve('docs/glide-app/(2018) Minerva Tour Stats and Scores.xlsx');
...
function parseCourseString(courseStr) {
  ...
  if (holeMatch) {
    ...
    if (inner.includes('front 9') || inner.includes('back 9') || inner === '9 holes') holeType = '9_holes';
...
async function run() {
  ...
  // Column indices for 2018 "Round Data":
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (scoreType === 'for stats' || scoreType === 'offseason') continue;

    const eventStr = r[4] ? r[4].toString().trim() : '';
    const parsedEvent = parseEventString(eventStr);
    if (!parsedEvent) continue; // Skip rows with no valid event
    ...
    const par = typeof r[16] === 'number' ? r[16] : parseInt(r[16]) || null;
  ...
  // Ensure 2018 season exists
  let { data: seasonRows } = await supabase.from('seasons').select('*').eq('year', SEASON_YEAR);
  if (!seasonRows || seasonRows.length === 0) {
    console.log(`Creating ${SEASON_YEAR} season...`);
    if (!DRY_RUN) {
      const { data: newSeason, error } = await supabase.from('seasons')
        .insert({ year: SEASON_YEAR, mode: 'off_season' })
        .select().single();
      if (error) { console.error(`Failed to create season: ${error.message}`); process.exit(1); }
      seasonRows = [newSeason];
    } else {
      console.log('  (skipped in dry run)');
      seasonRows = [{ id: 'dry-run', year: SEASON_YEAR }];
    }
  }
  ...
  for (const rd of roundData) {
    ...
    else console.log(`  Creating new course: ${rd.courseName} - ${rd.teeName} | R:${rd.rating} S:${rd.slope} P:${rd.par || 'default'}`);
    if (!DRY_RUN) {
      const defaultPar = rd.par || (rd.holeType === '9_holes' ? 36 : 72);
