...
const XLSX_PATH = resolve('docs/glide-app/(2021) Minerva Tour App.xlsx');
...
async function run() {
  ...
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i];
    const player = r[1];
    // Skip "For Stats" placeholder rows AND duplicate header row (Player="Player")
  ...
  for (const [eventNum, info] of [...eventMap.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    const isMajor = info.eventType === 'M' || info.eventType === 'C';
