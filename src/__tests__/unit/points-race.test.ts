...
describe('computePointsRace', () => {
  ...
  it('uses regular points for non-major events in scratch mode regardless of season position', () => {
    // Major status is determined solely by event.is_major in the database.
    // A regular event is never auto-promoted to major based on its position in the season.
    const events = [
      makeEvent({ id: 'e1', event_number: 1, is_major: false }),
      makeEvent({ id: 'e2', event_number: 2, is_major: false }),
      makeEvent({ id: 'e3', event_number: 3, is_major: false }),
    ];
    ...
    // Regular points, 3 participants: 1st=3, 2nd=2, 3rd=1
    expect(data[1].alice).toBe(3);
    expect(data[1].bob).toBe(2);
    expect(data[1].charlie).toBe(1);
  });

  it('uses major points for events with is_major=true in scratch mode', () => {
    const events = [
      makeEvent({ id: 'e1', event_number: 1, is_major: false }),
      makeEvent({ id: 'e2', event_number: 2, is_major: true }),
    ];
    ...
    const scores: Score[] = [
      makeScore({ id: 's1', user_id: 'alice', event_id: 'e1', net_strokes_over_par: -3, gross_score: 70 }),
      makeScore({ id: 's2', user_id: 'bob', event_id: 'e1', net_strokes_over_par: -1, gross_score: 75 }),
      makeScore({ id: 's3', user_id: 'alice', event_id: 'e2', net_strokes_over_par: -2, gross_score: 72 }),
      makeScore({ id: 's4', user_id: 'bob', event_id: 'e2', net_strokes_over_par: 0, gross_score: 78 }),
    ];
    ...
    // Event 2: major (is_major=true), 2 participants => 1st=max(round(2*1.33*10)/10,10)=10, 2nd=7
